...
fn parse_type(value: &str) -> Result<Type, Error> {
    match value.chars().nth(0) {
        ...
        Some(_) => Err(Error::Message(
            "cannot parse type : ".to_string() + &value.to_string(),
        )),
...
fn parse_transaction(currency: &str, line: &str) -> Result<Transaction, Error> {
    ...
    if parts.len() != 5 {
        return Err(Error::Message(
            "Invalid transaction line : ".to_string() + &line.to_string(),
        ));
    }
    Ok(Transaction {
...
fn parse_transactions(currency: &str, line: &str) -> Result<Vec<Transaction>, Error> {
    line.lines()
        .map(|str| str.trim())
        .map(|l| parse_transaction(currency, l))
        .collect()
}
...
impl Database {
    pub fn open(file_path: &str) -> Database {
        Database {
            handle: UnQLite::create(file_path),
        }
    }

    pub fn open_readonly(file_path: &str) -> Database {
        Database {
            handle: UnQLite::open_readonly(file_path),
        }
    }
...
mod tests {
    ...
    fn test_parse_transaction() {
        ...
        assert_eq!(
            ...
            Transaction {
                ...
                total_value: 15.,
                price: 30.,
                timestamp: "2023-03-27T08:00:00".to_string()
            }
        );
    }
    ...
    fn test_parse_transactions() {
        let currency = "BTC";
        let transactions =
            "B,0.5,15.000000,30.0,2023-01-01T08:00:00\nS,0.5,20.000000,40.0,2023-01-01T09:00:00";
